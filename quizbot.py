...
class QuizBot:
    ...
    def ask_question(self, update, context):
        question_data = context.chat_data['questions'][context.chat_data['question_index']]
    ...
    def end_quiz(self, update, context):
        ...
        self.bot.send_message(chat_id=update.effective_chat.id, text='Quiz complete! Your score is *{}*.'.format(score), parse_mode=telegram.ParseMode.MARKDOWN)
        self.save_score(update.effective_user.id, score, update.effective_user.first_name, update.effective_user.username)
    ...
    def save_score(self, user_id, score, name, username):
        with open('scores.json', 'r+') as f:
            high_scores = json.load(f)
            for user in high_scores:
                ...
                user_data = {'id': user_id, 'name': name, 'username': username, 'score': score}
    ...
    def next_question(self, update, context):
        if 'score' in context.chat_data:
            ...
            self.bot.send_message(chat_id=update.effective_chat.id, text='No active quiz.')
